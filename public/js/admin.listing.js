...
const AUTH_URL = "http://ec2-34-201-189-88.compute-1.amazonaws.com/auth/"; 
const ADMIN_URL = "http://ec2-34-201-189-88.compute-1.amazonaws.com/admin/"; 
...
function readQlFile(file, imagesArr, quill) {
  ...
  handleFileUpload("#put", async file => {
    ...
    await fetch(urlAndKey.url, {
      ...
      .then(data => {
        ...
        myAxios
          .post(API_URL + "storeimage/feature", storeImage)
          .then(resp => {
            console.log(resp);
  ...
  handleFileUpload("#otherimages", async file => {
    ...
    await fetch(urlAndKey.url, {
      ...
      .then(data => {
        ...
        myAxios
          .post(API_URL + "storeimage", storeImage)
          .then(resp => {
            console.log(resp);
  ...
  $(document).on("click", "button.feature-remove", function(e) {
    ...
    myAxios
      .delete(API_URL + `removeimage/feature/${clicked}`)
      .then(resp => {
        console.log(resp);
        $("#put").val("");
  ...
  $(document).on("click", "button.other-remove", function(e) {
    ...
    const clicked = $(this).attr("id");
    myAxios
      .delete(API_URL + `removeimage/${clicked}`)
      .then(resp => {
        console.log(resp);
        $("#otherimages").val("");
  ...
  $("body").on("click", "button.submit-button", function(e) {
    ...
    updates.listing.id = sessionStorage.getItem('currentListing')
      myAxios
        .put(API_URL + "stagelisting", updates.listing)
        .then(resp => {
            console.log(resp);
    ...
    updates.social_media.listing_id = sessionStorage.getItem('currentListing')
      myAxios
        .put(API_URL + "stagelisting/social_media", updates.social_media)
        .then(resp => {
          console.log(resp);
        })
        .catch(err => {
    ...
    if (updates.hours) {
      updates.social_media.listing_id = sessionStorage.getItem('currentListing')
        myAxios
          .put(API_URL + "stagelisting/hours", updates.hours)
          .then(resp => {
            console.log(resp);
          })
          .catch(err => {
      ...
      if (Object.values(updates.business_description).length && $(e.target).attr('id') === 'description-submit-button') {
          ...
          let uploadImages = await descriptionImages.map(async (image, index) => {
              ...
              await fetch(urlAndKey.url, {
                ...
                .then(data => {
                  ...
                  // };
                  // myAxios
                  //   .post(API_URL + "storeimage/delta", storeImage)
                  //   .then(resp => {
                  //     console.log(resp);
                  //   })
                  //   .catch(err => {
        ...
        if (currentListing) {
          ...
          console.log(updates.business_description)
            myAxios
              .put(API_URL + "updatedescription", updates.business_description)
              .then(resp => {
                console.log(resp); 
              })
              .catch(err => {
          ...
          console.log(updates.business_description)
            myAxios
              .put(API_URL + "updatedescription/staged", updates.business_description)
              .then(resp => {
                console.log(resp); 
              })
              .catch(err => {
